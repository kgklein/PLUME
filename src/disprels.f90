...
!                           Collin Brown, 2020
!------------------------------------------------------------------------------
subroutine fpc(wrootindex) !TODO: move this and all related routines to separate f90 file
  ...
  do is = 1, nspec
    ...
    do vperpindex = 0, numstepvperp
      ...
      do vparindex = 0, numstepvpar
        ...
        !call calc_correlation_perp(omega,ef,bf,vperpi,vpari,spec(is)%vv_s,spec(is)%Q_s,spec(is)%alph_s,spec(is)%tau_s,spec(is)%mu_s,Cor_s,fs1)
...
subroutine write_fs0()
  ...
  if(ex) then
    ...
    write(*,*)"Creating data folder for output..."
    write(*,*)'mkdir data'
    CALL system('mkdir data') !TODO: use more general pathing
    write(*,*)"Saving output to data folder..."
  endif
...
subroutine calc_fs1(omega,vperp,vpar,phi,ef,bf,V_s,q_s,aleph_s,tau_s,mu_s,aleph_r,fs0,fs1)
  ...
  real, dimension(1000) :: jbesselvals !array used to prevent repeated calculations !TODO: we can save a more time by making this a global 2d grid that stores values for (n,vperp) (might not be worth effort)
                                                                                     !TODO: make dynamic sizing (should crash at runtime if we overflow this array; still should prevent this)
